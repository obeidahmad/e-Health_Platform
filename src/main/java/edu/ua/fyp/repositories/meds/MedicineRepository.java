...
public interface MedicineRepository extends JpaRepository<Medicine, UUID>, JpaSpecificationExecutor<Medicine> {
	...
	default Page<Medicine> queryMedicines(QuerySettings querySettings) {
		return findAll(new Specification<Medicine>() {
			@Override
			public Predicate toPredicate(Root<Medicine> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
				...
				if (querySettings.getSearchText() != null) predicates.add(criteriaBuilder.like(
						criteriaBuilder.lower(root.get("brandName")), querySettings.getSearchText()
				));
